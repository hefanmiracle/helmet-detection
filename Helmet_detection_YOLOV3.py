...
def postprocess(frame, outs):
    ...
    if count_person >= 1:
        path = 'test_out/'
        frame_name=os.path.basename('test')             # trimm the path and give file name.
...
def dock(f):
    ...
    blob = cv.dnn.blobFromImage(f, 1/255, (inpWidth, inpHeight), [0,0,0], 1, crop=False)
    # Sets the input to the network
    net.setInput(blob)
    # Runs the forward pass to get output of the output layers
    outs = net.forward(getOutputsNames(net))
    # Remove the bounding boxes with low confidence
    f = postprocess(f, outs)
    return f


if __name__ == '__main__':
    for fn in glob('test_in/*.jpg'):
        frame = cv.imread(fn)
        frame_count =0
        print(frame)
        # Create a 4D blob from a frame.
